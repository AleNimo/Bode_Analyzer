...
void Screen1View::DataDisplay(float* freq,float*mag,float*phase,unsigned int num_points)
{
	/*
	Unicode::strncpy(textArea1Buffer, "HOLA", strlen("HOLA"));
	...
	*/
	...
	static float range_max = 1000000;
	static float range_min = 1;
	...
	if(plot == true)
	{
		...
		if(!first_time)
		{
			...
			for(i = 0;i<decades;i++)
			{
				...
				Unicode::strncpy(freq_label_buffer[i], freq_string, 4);
				//textArea1.resizeToCurrentText();
				freq_label[i]->setX((320-30)/decades*(i)+30-10);
		...
		if(type_plot == MAG)
			...
			for(unsigned int j = 0;j<num_points;j++)
			{
				...
				float f = log10((double)(100*freq[j]))-2;
			...
			for(unsigned int j = 0;j<num_points;j++)
			{
				...
				float f = log10((double)(100*freq[j]))-2;
				graphMAG.addDataPoint((float)f, (float)phase[j]);
				//graphPH.addDataPoint((float)f, (float)phase[j]);
			}
